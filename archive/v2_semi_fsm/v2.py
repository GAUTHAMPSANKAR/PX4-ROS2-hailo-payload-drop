"""
Prototype v2 – Semi-FSM architecture.

Still used the landing proxy behavior for testing, but introduced:

- Structured internal phases (pseudo-state-machine)
- Better lost-target handling
- More stable alignment and descent logic

This version moved significantly closer to the final mission behavior.
"""
...
class BullseyeDetector(Node):
    def __init__(self):
        ...
        self.cmd_pos_pub = self.create_publisher(PositionTarget, '/mavros/setpoint_raw/local', 10)
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameters(
            ...
            parameters=[ # type: ignore[arg-type]
                ...
                ('vision.threshold_pixels', 5),
                ('vision.detection_confidence', 0.7),
                ('vision.max_frames_lost', 15),
                ('mission.hold_duration', 10.0),
                ('mission.descent_altitude', 10.0),
                ('mission.detection_history_size', 15),
                ('control.exponential_weight', -2.0)
            ]
        )
    ...
    def _initialize_variables(self):
        ...
        self.Kp = self.get_parameter('control.kp_xy').value
        self.Kpd = self.get_parameter('control.kp_z').value
        self.mvel = self.get_parameter('control.max_velocity').value
        self.threshold = self.get_parameter('vision.threshold_pixels').value
        self.min_confidence = self.get_parameter('vision.detection_confidence').value
        self.max_frames_lost = self.get_parameter('vision.max_frames_lost').value
        self.hold_duration = self.get_parameter('mission.hold_duration').value
        self.descent_altitude = self.get_parameter('mission.descent_altitude').value
        self.eweight = self.get_parameter('control.exponential_weight').value
        detection_history_size = self.get_parameter('mission.detection_history_size').value
    ...
    def hold_state(self):
        ...
        if elapsed >= self.hold_duration:    #type: ignore
            self.get_logger().info("Hold duration complete")
            self.hold_started = 2
        else:
            return
    ...
    def _update_stable_target(self):
        ...
        self.last_known_target_pos = self.current_position if self.current_position else None
